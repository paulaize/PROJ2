...
- Study metadata validation outputs under `reports/qc/`, generated by
  `scripts/qc/build_study_metadata.py` while the editable CSV lives under
  `derivatives/manifests/study_metadata.csv`.
- Candidate brain-mask manifests under `reports/qc/`, including model
  prediction validation outputs from `scripts/masks/build_brain_mask_manifest.py`.
- Brain-mask post-processing summaries under `reports/qc/`, generated by
...
- Manifest-driven scan role, mask path, side, lesion, and inclusion/exclusion
  overrides through `derivatives/manifests/study_metadata.csv`, merged into
  the QC-gated `derivatives/manifests/analysis_manifest.csv`.
