...
scripts/qc/build_study_metadata.py
...
src/lys_bbb/study_metadata.py    # editable side/group/lesion/review metadata builder
...
- Keep editable study-level fields in
  `derivatives/manifests/study_metadata.csv`, generated by
  `scripts/qc/build_study_metadata.py`; rebuild the analysis manifest with
  `--metadata-manifest` so group, ipsilateral side, lesion masks, and review
  decisions flow into cohort quantification without bypassing QC gates.
